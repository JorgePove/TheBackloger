...
// Reads ?year=XXXX from the URL. Falls back to the active year stored in
// localStorage, then to the current calendar year.
function getActiveYear() {
  const params = new URLSearchParams(window.location.search);
  const fromUrl = parseInt(params.get('year'));
  if (fromUrl && !isNaN(fromUrl)) return fromUrl;
  const stored = parseInt(localStorage.getItem('trackerino_active_year'));
  if (stored && !isNaN(stored)) return stored;
  return new Date().getFullYear();
}

function storageKey(year) {
  // Keep backward compat: if year matches old key's implicit year we still
  // write to the new namespaced key so everything is unified.
  return `mediatracker_${year}_v1`;
}

// Legacy migration: if the old key exists and the new one doesn't, migrate once.
(function migrateOnce() {
  const year = new Date().getFullYear();
  const oldKey = 'mediatracker_v1';
  const newKey = storageKey(year);
  try {
    const oldData = localStorage.getItem(oldKey);
    const newData = localStorage.getItem(newKey);
    if (oldData && !newData) {
      localStorage.setItem(newKey, oldData);
      // Don't remove old key to preserve backward compat with old pages
    }
  } catch (_) {}
})();

const STORAGE_KEY = storageKey(getActiveYear()); // kept for any third-party ref

function loadData() {
  try {
    return JSON.parse(localStorage.getItem(storageKey(getActiveYear()))) || { items: [] };
  } catch { return { items: [] }; }
}

function saveData(data) {
  localStorage.setItem(storageKey(getActiveYear()), JSON.stringify(data));
}
...
async function searchTMDB(query, mediaType = 'movie') {
  const url = `https://api.themoviedb.org/3/search/${mediaType}?api_key=${API_KEYS.tmdb}&query=${encodeURIComponent(query)}&language=es-ES&page=1`;
...
async function searchBooks(query) {
  const url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(query)}&maxResults=20&key=${API_KEYS.googleBooks}`;
  const res = await fetch(url);
  const data = await res.json();
...
async function searchGames(query) {
  const url = `https://api.rawg.io/api/games?key=${API_KEYS.rawg}&search=${encodeURIComponent(query)}&page_size=20`;
  const res = await fetch(url);
  const data = await res.json();
  return (data.results || []).slice(0, 20).map(r => ({
    apiId: r.id,
...
// ── CREATOR / STUDIO SEARCH ───────────────────────────────────────────────────
// Returns the label for the "by creator" mode per type
function creatorLabel(type) {
  return { pelicula: 'Director o Actor', serie: 'Director o Actor', anime: 'Estudio', libro: 'Autor', videojuego: 'Desarrolladora' }[type] || 'Autor';
}

// Search TMDB by person, then fetch their movie/tv credits
async function searchTMDBByPerson(query, mediaType) {
  const personUrl = `https://api.themoviedb.org/3/search/person?api_key=${API_KEYS.tmdb}&query=${encodeURIComponent(query)}&language=es-ES`;
  const res  = await fetch(personUrl);
  const data = await res.json();
  const people = (data.results || []).slice(0, 5);
  if (!people.length) return [];

  // Fetch credits for first matching person
  const person = people[0];
  const creditsUrl = `https://api.themoviedb.org/3/person/${person.id}/${mediaType === 'movie' ? 'movie_credits' : 'tv_credits'}?api_key=${API_KEYS.tmdb}&language=es-ES`;
  const cRes  = await fetch(creditsUrl);
  const cData = await cRes.json();
  const works = [...(cData.cast || []), ...(cData.crew || [])]
...
// Search anime by studio via Jikan producers endpoint
async function searchAnimeByStudio(query) {
  const url = `https://api.jikan.moe/v4/producers?q=${encodeURIComponent(query)}&limit=5`;
  const res  = await fetch(url);
  const data = await res.json();
  const producers = (data.data || []).slice(0, 3);
  if (!producers.length) return [];

  const producer = producers[0];
  const animeUrl = `https://api.jikan.moe/v4/anime?producers=${producer.mal_id}&limit=20&order_by=popularity`;
  const aRes  = await fetch(animeUrl);
  const aData = await aRes.json();
  return (aData.data || []).slice(0, 20).map(r => ({
    apiId: r.mal_id,
    title: r.title,
    year: r.year || '',
    poster: r.images?.jpg?.image_url || null,
    overview: r.synopsis || '',
    author: producer.titles?.[0]?.title || producer.name || query,
  }));
}

// Search books by author using Google Books inauthor: syntax
async function searchBooksByAuthor(query) {
  const url = `https://www.googleapis.com/books/v1/volumes?q=inauthor:${encodeURIComponent(query)}&maxResults=20&orderBy=relevance&key=${API_KEYS.googleBooks}`;
  const res  = await fetch(url);
  const data = await res.json();
  return (data.items || []).slice(0, 20).map(r => ({
    apiId: r.id,
...
// Search games by developer/publisher via RAWG
async function searchGamesByDeveloper(query) {
  // Try developers first, fall back to publishers
  const devUrl = `https://api.rawg.io/api/developers?key=${API_KEYS.rawg}&search=${encodeURIComponent(query)}&page_size=5`;
  const devRes  = await fetch(devUrl);
  const devData = await devRes.json();
  const devs = (devData.results || []);

  if (devs.length) {
    const dev = devs[0];
    const gamesUrl = `https://api.rawg.io/api/games?key=${API_KEYS.rawg}&developers=${dev.id}&page_size=20&ordering=-rating`;
    const gRes  = await fetch(gamesUrl);
    const gData = await gRes.json();
    return (gData.results || []).slice(0, 20).map(r => ({
      apiId: r.id,
      title: r.name,
      year: (r.released || '').slice(0, 4),
      poster: r.background_image || null,
  ...
  // Fallback: try publishers
  const pubUrl = `https://api.rawg.io/api/publishers?key=${API_KEYS.rawg}&search=${encodeURIComponent(query)}&page_size=5`;
  const pubRes  = await fetch(pubUrl);
  const pubData = await pubRes.json();
  const pubs = (pubData.results || []);
  if (!pubs.length) return [];

  const pub = pubs[0];
  const gamesUrl = `https://api.rawg.io/api/games?key=${API_KEYS.rawg}&publishers=${pub.id}&page_size=20&ordering=-rating`;
  const gRes  = await fetch(gamesUrl);
  const gData = await gRes.json();
  return (gData.results || []).slice(0, 20).map(r => ({
    apiId: r.id,
...
async function searchByCreator(type, query) {
  if (type === 'pelicula') return searchTMDBByPerson(query, 'movie');
  if (type === 'serie')    return searchTMDBByPerson(query, 'tv');
  if (type === 'anime')    return searchAnimeByStudio(query);
  if (type === 'libro')    return searchBooksByAuthor(query);
  if (type === 'videojuego') return searchGamesByDeveloper(query);
  return [];
}
...
function avgScore(items) {
  const scored = items.filter(i => i.score && parseFloat(i.score) > 0);
  if (!scored.length) return null;
  const sum = scored.reduce((acc, i) => acc + parseFloat(i.score), 0);
...
function exportData() {
  ...
  const data = {
    ...
    data: {
      [year]: {
        media: loadData(),
        tareas: (() => {
          try { return JSON.parse(localStorage.getItem(`trackerino_tareas_${year}_v1`)) || { lists: [] }; } catch { return { lists: [] }; }
        })(),
      }
    }
  };
  ...
  a.href = url;
  a.download = `eltrackerino-${year}-backup-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (parsed.version === 2 && parsed.data) {
          const yearData = parsed.data[year];
          if (!yearData) { toast(`El backup no contiene datos para ${year}`, 'error'); return; }
          if (!confirm(`¿Importar backup del año ${year}? Se reemplazarán sus datos.`)) return;
          if (yearData.media) localStorage.setItem(storageKey(year), JSON.stringify(yearData.media));
          if (yearData.tareas) localStorage.setItem(`trackerino_tareas_${year}_v1`, JSON.stringify(yearData.tareas));
          toast('Backup importado correctamente ✓', 'success');
          if (typeof renderPage === 'function') setTimeout(renderPage, 100);
          updateSidebarCounts();
          return;
        }
        // Legacy v1 format
        const mediaData  = parsed.mediatracker || parsed;
        const tareasData = parsed.tareas || null;
        if (!Array.isArray(mediaData.items)) throw new Error('Formato inválido');
        if (!confirm(`¿Importar backup en el año ${year}? Se reemplazarán sus datos.`)) return;
        saveData(mediaData);
        if (tareasData && Array.isArray(tareasData.lists)) {
          localStorage.setItem(`trackerino_tareas_${year}_v1`, JSON.stringify(tareasData));
        }
        toast('Backup importado correctamente ✓', 'success');
        if (typeof renderPage === 'function') setTimeout(renderPage, 100);
        updateSidebarCounts();
...
function updateSidebarCounts() {
  ...
  // Show active year in sidebar
  const yearEl = document.getElementById('sidebar-year-label');
  if (yearEl) yearEl.textContent = getActiveYear();
}
...
function openScorePopup(onDone) {
  ...
  range.addEventListener('input', () => {
    ...
    display.textContent = v > 0 ? v.toFixed(1) + ' / 10' : 'Sin puntuación';
  ...
  document.getElementById('score-confirm-btn').addEventListener('click', () => {
    const v = parseFloat(range.value);
    // Only save a score if user explicitly moved the slider to a value > 0
    closeScorePopup(hasInteracted && v > 0 ? v.toFixed(1) : null, onDone);
  });
...
function moveItem(id, newStatus) {
  ...
  if (newStatus === 'abandoned') {
    if (!confirm('¿Seguro que quieres marcar esto como abandonado?')) return;
  }

  const updates = { status: newStatus };
  if (newStatus === 'progress') {
    // Only set startedAt if it hasn't been set before (don't overwrite on resume)
    const existing = loadData().items.find(i => i.id === id);
...
function removeItem(id) {
  ...
  const name = item ? `"${item.title}"` : 'este título';
...
function openModal(type, editId) {
  ...
  // Update creator button label for this type
  const btnCreator = document.getElementById('search-mode-creator');
  if (btnCreator) btnCreator.textContent = creatorLabel(type);
  ...
  if (editId) {
    ...
    if (!item.apiId) {
      // Item custom: abrir directamente en modo manual, con los campos rellenos
      setManualMode(true);
      document.getElementById('manual-title').value  = item.title || '';
      document.getElementById('manual-year').value   = item.year || '';
      document.getElementById('manual-author').value = item.author || '';
      const isDataUrl = (item.poster || '').startsWith('data:');
      document.getElementById('manual-poster-url').value = isDataUrl ? '' : (item.poster || '');
      _manualPosterDataUrl = isDataUrl ? item.poster : null;
      renderManualPosterPreview(item.poster || '');
    } else {
      overlay._selected = {
        title: item.title,
        year: item.year,
...
function resetModal() {
  const overlay = document.getElementById('add-modal');
  if (!overlay) return;
  ['api-search-input','field-score','field-notes','manual-title','manual-year','manual-author','manual-poster-url'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  const fileEl = document.getElementById('manual-poster-file');
  if (fileEl) fileEl.value = '';
  _manualPosterDataUrl = null;
  renderManualPosterPreview('');
  setManualMode(false);
  const datesWrap = document.getElementById('modal-dates-wrap');
  ...
  // Reset search mode to title
  _searchMode = 'title';
  const btnTitle   = document.getElementById('search-mode-title');
  const btnCreator = document.getElementById('search-mode-creator');
  const label      = document.getElementById('search-field-label');
  if (btnTitle)   btnTitle.classList.add('active');
  if (btnCreator) btnCreator.classList.remove('active');
  if (label)      label.textContent = 'Buscar título';
  const input = document.getElementById('api-search-input');
  if (input) input.placeholder = 'Escribe para buscar...';
}

let _searchTimeout = null;
let _searchMode = 'title'; // 'title' | 'creator'
let _manualPosterDataUrl = null; // base64 de la imagen subida a mano, si la hay

// ── MODO MANUAL (item custom) ─────────────────────────────────────────────────
function toggleManualMode() {
  const overlay = document.getElementById('add-modal');
  const isManual = overlay.dataset.manualMode === 'true';
  setManualMode(!isManual);
}

function setManualMode(manual) {
  const overlay = document.getElementById('add-modal');
  overlay.dataset.manualMode = manual ? 'true' : 'false';
  const type = overlay.dataset.type;

  document.getElementById('search-entry-block').style.display   = manual ? 'none' : 'block';
  document.getElementById('manual-entry-fields').style.display  = manual ? 'flex' : 'none';
  document.getElementById('manual-toggle-btn').textContent = manual
    ? '🔍 Volver a buscar en base de datos'
    : '✎ ¿No lo encuentras? Añadir manualmente';
  document.getElementById('manual-author-label').textContent = creatorLabel(type);
}
...
function handleManualFileUpload(e) {
  ...
  reader.onload = (ev) => {
    const img = new Image();
    img.onload = () => {
      // Redimensiona/comprime para no inflar el almacenamiento (localStorage + Firestore)
      const maxW  = 300;
      ...
      _manualPosterDataUrl = canvas.toDataURL('image/jpeg', 0.75);
      document.getElementById('manual-poster-url').value = '';
      renderManualPosterPreview(_manualPosterDataUrl);
    };
...
function previewManualPosterFromUrl() {
  _manualPosterDataUrl = null; // si escribe una URL, descarta cualquier archivo subido antes
  const url = document.getElementById('manual-poster-url').value.trim();
  renderManualPosterPreview(url);
}

function renderManualPosterPreview(src) {
  const preview = document.getElementById('manual-poster-preview');
  preview.innerHTML = src ? `<img src="${src}" alt="preview">` : '';
}

function buildManualSelected() {
  const title = document.getElementById('manual-title').value.trim();
  if (!title) { toast('Escribe al menos un título', 'error'); return null; }
  const year   = document.getElementById('manual-year').value.trim();
  const author = document.getElementById('manual-author').value.trim();
  const urlVal = document.getElementById('manual-poster-url').value.trim();
  const poster = _manualPosterDataUrl || urlVal || null;
  return { title, year, author, poster, apiId: null, overview: '', pages: '' };
}
...
function initModalSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    _searchTimeout = setTimeout(async () => {
      const type = document.getElementById('add-modal').dataset.type;
      try {
        const results = _searchMode === 'creator'
          ? await searchByCreator(type, q)
          : await searchByType(type, q);
        renderApiResults(results, type);
      } catch (e) {
        resultsEl.innerHTML = '<div style="padding:8px;color:var(--red);font-size:0.8rem;">Error al buscar. Comprueba tu conexión o API key.</div>';
      }
    }, 500);
  });
}
...
function saveFromModal(targetStatus) {
  const overlay = document.getElementById('add-modal');
  const isManual = overlay.dataset.manualMode === 'true';
  const selected = isManual ? buildManualSelected() : overlay._selected;
  if (!selected) {
    if (!isManual) toast('Busca y selecciona un título primero', 'error');
    return;
  }
  const type = overlay.dataset.type;
  const item = {
...
function saveEditFromModal() {
  ...
  const selected = isManual ? buildManualSelected() : overlay._selected;
  if (!selected) {
    if (!isManual) toast('Selecciona un título', 'error');
    return;
  }
  ...
  <div class="modal-overlay" id="add-modal" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      ...
      <div class="modal-body">
        <div id="search-entry-block">
          <div class="form-group">
            <div class="search-mode-row">
              <label id="search-field-label">Buscar título</label>
              <div class="search-mode-toggle">
                <button id="search-mode-title" class="search-mode-btn active" onclick="setSearchMode('title')">Título</button>
                <button id="search-mode-creator" class="search-mode-btn" onclick="setSearchMode('creator')">Autor</button>
              </div>
            </div>
        ...
        <div class="manual-toggle-row">
          <button type="button" class="link-btn" id="manual-toggle-btn" onclick="toggleManualMode()">✎ ¿No lo encuentras? Añadir manualmente</button>
        </div>

        <div id="manual-entry-fields" style="display:none">
          <div class="form-group">
            <label>Título *</label>
            <input type="text" id="manual-title" placeholder="Título del ítem">
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Año</label>
              <input type="text" id="manual-year" placeholder="2024">
            </div>
            <div class="form-group">
              <label id="manual-author-label">Autor</label>
              <input type="text" id="manual-author" placeholder="">
            </div>
          </div>
          <div class="form-group">
            <label>Imagen (opcional)</label>
            <div class="manual-poster-row">
              <input type="text" id="manual-poster-url" placeholder="Pega una URL de imagen…" oninput="previewManualPosterFromUrl()">
              <span class="manual-poster-or">o</span>
              <input type="file" id="manual-poster-file" accept="image/*" onchange="handleManualFileUpload(event)">
            </div>
            <div id="manual-poster-preview" class="manual-poster-preview"></div>
          </div>
        </div>
...
const _colSearch = { 'col-pending': '', 'col-progress': '', 'col-completed': '' };

function initColSearch() {
  ['col-pending', 'col-progress', 'col-completed'].forEach(colId => {
    const input = document.getElementById(`search-${colId}`);
    if (!input) return;
    input.addEventListener('input', () => {
      _colSearch[colId] = input.value.trim().toLowerCase();
      // Re-render only that column from the current page data
      if (typeof _reRenderKanbanCol === 'function') _reRenderKanbanCol(colId);
    });
  });
}

// Called by each category page after renderPage — wires up search + stores items per col
const _colItems = {};
function registerColItems(colId, items) {
  _colItems[colId] = items;
}

function _reRenderKanbanCol(colId) {
  const allItems = _colItems[colId];
  if (!allItems) return;
  const q = _colSearch[colId];
  const filtered = q
    ? allItems.filter(i =>
  ...
  renderCol(colId, filtered);
  // Update visible count badge (show filtered/total)
  const countEl = document.getElementById(`count-${colId.replace('col-', '')}`);
  if (countEl) {
    countEl.textContent = q ? `${filtered.length}/${allItems.length}` : allItems.length;
  }
  if (typeof initAllDrag === 'function') initAllDrag();
}
...
function _kanbanPointerUp(e) {
  if (!_kDrag) return;
  const { dragEl, ghost, placeholder, col } = _kDrag;
  placeholder.replaceWith(dragEl);
  dragEl.classList.remove('dragging');
  ghost.remove();
  _saveKanbanOrder(col);
  _kDrag = null;
...
function _saveKanbanOrder(col) {
  const orderedIds = [...col.querySelectorAll('.kanban-card')].map(el => el.dataset.id);
  const data = loadData();
  const idxMap = {};
  orderedIds.forEach((id, i) => { idxMap[id] = i; });
  // Sort only the items that belong to this column; leave others untouched
  const colItems = data.items.filter(i => idxMap[i.id] !== undefined);
  const otherItems = data.items.filter(i => idxMap[i.id] === undefined);
  colItems.sort((a, b) => idxMap[a.id] - idxMap[b.id]);
  // Rebuild: place colItems back in their original positions in the full array
  const positions = data.items
    .map((item, idx) => idxMap[item.id] !== undefined ? idx : -1)
    .filter(idx => idx !== -1);
  positions.forEach((pos, i) => { data.items[pos] = colItems[i]; });
  saveData(data);
}
...
function openDetailPopup(id) {
  ...
  <div class="dp-overlay" id="detail-popup-overlay" onclick="if(event.target===this)closeDetailPopup()">
    ...
    <div class="dp-modal">
      ...
      <div class="dp-body">
        ...
        <!-- Score — editable -->
        <div class="dp-section">
          <div class="dp-section-label">Puntuación</div>
          <div class="dp-score-edit">
            <input type="range" class="dp-score-range" id="dp-score-range-${id}"
                   min="0" max="10" step="0.5" value="${item.score || 0}">
            <span class="dp-score-val" id="dp-score-val-${id}">${item.score ? item.score + ' / 10' : 'Sin puntuación'}</span>
            <button class="dp-save-notes" onclick="saveDetailScore('${id}')">💾 Guardar puntuación</button>
          </div>
        </div>
      </div>
    </div>
  ...
  // Wire score range live display
  const scoreRange = document.getElementById(`dp-score-range-${id}`);
  const scoreVal   = document.getElementById(`dp-score-val-${id}`);
  if (scoreRange && scoreVal) {
    scoreRange.addEventListener('input', () => {
      const v = parseFloat(scoreRange.value);
      scoreVal.textContent = v > 0 ? v.toFixed(1) + ' / 10' : 'Sin puntuación';
      scoreVal.style.color = v >= 7 ? 'var(--green)' : v >= 5 ? 'var(--accent)' : v > 0 ? 'var(--orange)' : 'var(--text3)';
    });
    // Set initial color
    const iv = parseFloat(scoreRange.value);
    scoreVal.style.color = iv >= 7 ? 'var(--green)' : iv >= 5 ? 'var(--accent)' : iv > 0 ? 'var(--orange)' : 'var(--text3)';
  }
}
...
function saveDetailScore(id) {
  const range = document.getElementById(`dp-score-range-${id}`);
  if (!range) return;
  const v = parseFloat(range.value);
  const score = v > 0 ? v.toFixed(1) : '';
  updateItem(id, { score });
  toast(score ? `Puntuación ${score} guardada ★` : 'Puntuación eliminada', 'success');
  if (typeof renderPage === 'function') renderPage();
}
